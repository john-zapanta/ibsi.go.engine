...
func DeleteUpdate(w http.ResponseWriter, r *http.Request, dsh CrudHandler) {
	...
	if cmd, ok := dbase.Connections[un].GetCommand(uc); ok {
		...
		if dsh.OnInitCrudParams != nil {
			dsh.OnInitCrudParams("delete", params, w, r)
		}
		...
		if qry, err := cmd.Execute(params); err != nil {
			...
			if output, err := qry.GetOutput(); err == nil {
				...
				kv.Set("deleted", append([]dbase.TDataTableOutput{}, result))
			} else {
